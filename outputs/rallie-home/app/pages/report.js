...
export function renderReportPage({ state }) {
  ...
  return `<header class="report-flow-top-bar">
      <button type="button" data-action="navigate-back" data-fallback-action="report-name" aria-label="Back to your name">${icon("back")}</button>
      <span>Report status</span>
      <span class="step-count">2 of 2</span>
    </header>
    <div class="report-details-page page-scroll">
      <div class="step-progress" aria-label="Step 2 of 2"><span></span><span class="active"></span></div>
      <span class="eyebrow">QUICK COURT CHECK</span>
      <h1>What does the court look like right now?</h1>
      <p class="report-lede">A few quick details help nearby players decide when to go.</p>
      <section class="gps-validate-row ${state.gpsEnabled ? "enabled" : ""}">
        <button type="button" data-action="toggle-gps" aria-pressed="${state.gpsEnabled}" aria-label="Validate with GPS">
          ${icon("check")}<span><strong>Validate with GPS</strong><small>Build trust in your report</small></span><span class="gps-switch"></span>
        </button>
        ${state.gpsMessage ? `<p class="gps-message ${state.gpsStatus}" role="status" aria-live="polite">${escapeHtml(state.gpsMessage)}</p>` : ""}
      </section>
      <section class="report-card">
        <span class="question-number">01</span><h2>Just arrived or wrapping up?</h2>
        <div class="segmented">
          ${["Just got here", "Heading out"].map((label) => optionButton({ label, active: state.arrival === label, action: "arrival" })).join("")}
        </div>
      </section>
      <section class="report-card">
        <span class="question-number">02</span><h2>How many courts are open?</h2>
        <div class="counter">
          <button type="button" data-action="counter" data-direction="-1" aria-label="Decrease open courts" ${state.openCourts === 0 ? "disabled" : ""}>${icon("minus")}</button>
          <div><strong>${state.openCourts}</strong><span>of ${court.total} courts</span></div>
          <button type="button" data-action="counter" data-direction="1" aria-label="Increase open courts" ${state.openCourts >= court.total ? "disabled" : ""}>${icon("plus")}</button>
        </div>
      </section>
      ${occupied ? `<section class="report-card">
        <span class="question-number">03</span><h2>How many parties are waiting?</h2>
        <div class="party-row">
          ${partyOptions({ occupied, waitingParties: state.waitingParties })}
        </div>
      </section>` : `<div class="queue-cleared-note" role="status">${icon("check")} No waiting parties while a court is open.</div>`}
    </div>
    <footer class="fixed-cta">
      <button class="submit-report" type="button" data-action="submit-report" ${state.isOffline || state.gpsStatus === "too-far" || state.reportSubmitStatus === "submitting" ? "disabled" : ""}>
        ${state.isOffline ? "Reconnect to submit" : state.reportSubmitStatus === "submitting" ? "Publishing your report…" : "Submit Report"}
      </button>
      ${state.reportSubmitMessage ? `<p class="report-submit-message ${state.reportSubmitStatus}" role="alert">${escapeHtml(state.reportSubmitMessage)}</p>` : ""}
      <p class="report-note">Your report is anonymous to other players except for the name you entered.</p>
    </footer>`;
}

function reporterNamePage({ state, court }) {
  return `<header class="report-name-top-bar">
      <button type="button" data-action="navigate-back" data-fallback-action="court-detail" aria-label="Back">${icon("back")}</button>
      <span>Report status</span>
      <span class="step-count">1 of 2</span>
    </header>
    <form id="reporterNameForm" class="report-name-page page-scroll" data-submit-action="save-reporter-name">
      <div class="step-progress" aria-label="Step 1 of 2"><span class="active"></span><span></span></div>
      <span class="eyebrow">COMMUNITY REPORT</span>
      <h1>Know before you go.<br /><span>Help the next player.</span></h1>
      <p class="report-lede">Your update keeps court availability useful for everyone.</p>
      <h2>Reporting for</h2>
      <section class="report-name-court">${icon("pin")}<span>${escapeHtml(court.reportName)}</span></section>
      <section class="report-name-card">
        <label for="reporterName">What should we call you?</label>
        <div class="report-name-input">
          <input id="reporterName" name="reporterName" autocomplete="name" placeholder="Enter your name" value="${escapeHtml(state.reporterName)}" />
          ${icon("user")}
        </div>
        ${state.reporterNameMessage ? `<p class="report-name-error" role="alert">${escapeHtml(state.reporterNameMessage)}</p>` : ""}
      </section>
    </form>
    <footer class="fixed-cta">
      <button class="report-name-next" type="submit" form="reporterNameForm">Continue ${icon("chevron")}</button>
      <p class="report-note">Your name appears with your report and identifies your party when courts are full.</p>
    </footer>`;
}
...
function confirmationPage() {
  return `<header class="report-flow-top-bar">
      ...
      <span>Report complete</span><span class="step-count">Done</span>
    </header>
    <section class="confirmation-page page-scroll">
      <div class="confirmation-content">
        ...
        <span class="success-pill">${icon("check")} Report published</span>
        <h1>Score! You helped someone play sooner.</h1>
        <p>Your update is now part of the live court status.</p>
      </div>
      <footer class="fixed-cta">
        <button type="button" data-action="finish-report">Back to court details ${icon("chevron")}</button>
        <p class="report-note">Thanks for keeping Rallie useful.</p>
      </footer>
    </section>`;
}
...
function partyOptions({ occupied, waitingParties }) {
  ...
  return labels.map((label, index) => {
    ...
    return `<button class="${active ? "active" : ""}" type="button" data-action="parties" data-value="${value}" aria-pressed="${active}">${label}</button>`;
  }).join("");
}

function optionButton({ label, active, action }) {
  return `<button class="${active ? "active" : ""}" type="button" data-action="${action}" data-value="${escapeHtml(label)}" aria-pressed="${active}">${escapeHtml(label)}</button>`;
}
